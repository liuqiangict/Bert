...
flags.DEFINE_string('input_training_data_path', '', 'training data dir')

tf.app.flags.DEFINE_string('input_validation_data_path', '', 'validation data dir')

flags.DEFINE_string("labels", "0,1", "label set")
...
flags.DEFINE_string("vocab_file", "./Config/vocab.txt", "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string("bert_config_file", "./Config/bert_config.json", "The config json file corresponding to the pre-trained BERT model. This specifies the model architecture.")

flags.DEFINE_string("task_name", "MRPC", "The name of the task to train.")

flags.DEFINE_string("output_model_path", '', "The output directory where the model checkpoints will be written.")
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          predictions={"guid" : guid, "probabilities": probabilities},
          scaffold_fn=scaffold_fn)
    return output_spec
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    train_examples = processor.get_train_examples(FLAGS.input_training_data_path)
    num_train_steps = int(
  ...
  if FLAGS.do_train:
    train_file = os.path.join(FLAGS.output_model_path, "train.tf_record")
  ...
  if FLAGS.do_eval:
    ...
    eval_examples = processor.get_dev_examples(FLAGS.input_validation_data_path)
    ...
    eval_file = os.path.join(FLAGS.output_model_path, "eval.tf_record")
    file_based_convert_examples_to_features(
    ...
    output_eval_file = os.path.join(FLAGS.output_model_path, "eval_results.txt")
  ...
  if FLAGS.do_predict:
    predict_examples = processor.get_test_examples(FLAGS.input_validation_data_path)
    ...
    predict_file = os.path.join(FLAGS.output_model_path, "predict.tf_record")
    ...
    output_predict_file = os.path.join(FLAGS.output_model_path, "predict.tsv")
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      for (i, prediction) in enumerate(result):
        ...
        if i >= num_actual_predict_examples:
          raise Exception('Index are out of num_actual_predict_examples, wired heeee.')
